...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Salom Senior QA! Muammoni yozing, men uni Jira formatiga keltiraman:",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Qaysi tilda hisobot kerak?", reply_markup=reply_markup
    )
    ...
    try:
        response = await asyncio.to_thread(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"You are a Senior QA. Write a detailed Jira report in {selected_lang}. Include Summary, Steps, Expected, and Actual results. Use plain text only.",
                },
                {
                    "role": "user",
                    "content": f"Format this into a {selected_lang} report: {raw_text}",
                },
            ],
            temperature=0.1,
        )
        ...
        await query.edit_message_text(
            f"📋 **Hisobot ({selected_lang.capitalize()}):**\n\n{report}",
            reply_markup=InlineKeyboardMarkup(keyboard),
        )
...
async def jira_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        selected_type = None
        for t in ["Bug", "Ошибка", "Task", "Story", "Issue", "Error"]:
        ...
        # Xatolikni f-string xatosiz chiqarish
        types_str = ", ".join(available_types) if available_types else "Nomalum"
        await query.message.reply_text(
            f"❌ Jira xatosi: {str(e)}\nMavjud turlar: {types_str}"
        )
